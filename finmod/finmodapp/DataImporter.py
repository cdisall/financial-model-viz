...
class DataImporter(object):
    ...
    def get_close_data(self, sym):
         """  		  	   		  		 			  		 			 	 	 		 		 	
        Get data for stock with ticker symbol sym		  	   		  		 			  		 			 	 	 		 		 	
  		  	   		  		 			  		 			 	 	 		 		 	
        :param sym: Ticker symbol
        :type sym: str		  	   		  		 			  		 			 	 	 		 		 	
        :output self.get_ndarray(data): 2D ndarray with date and closing value for the stock (adjusted) 		  	   		  		 			  		 			 	 	 		 		 	
        :type self.get_ndarray(data): numpy.ndarray  		  	   		  		 			  		 			 	 	 		 		 	
        """  
         data, meta = ts.get_daily_adjusted(sym)
         return self.get_ndarray(data)[:, [0, 3]]

if __name__ == '__main__':
    d = DataImporter()
    sym = 'GOOGL'
    print(d.get_close_data(sym)[:4])
